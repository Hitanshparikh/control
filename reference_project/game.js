...
import { SettingsManager } from './SettingsManager.js'; // Import SettingsManager
import { ModelLibraryManager } from './ModelLibraryManager.js'; // Import ModelLibraryManager
export var Game = /*#__PURE__*/ function() {
    ...
    function Game(renderDiv) {
        ...
        this.settingsManager = null; // Settings manager instance
        this.modelLibraryManager = null; // Model library manager instance
                    ...
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            ...
                            case 3:
                                ...
                                // Initialize Settings and Model Library managers
                                _this.settingsManager = new SettingsManager(_this);
                                _this.modelLibraryManager = new ModelLibraryManager(_this);
        ...
        },
        {
            ...
            value: function _setupSpeechRecognition() {
                ...
                this.speechManager = new SpeechManager(function(finalTranscript, interimTranscript) {
                    ...
                    if (_this.settingsManager && !_this.settingsManager.isVoiceControlEnabled()) {
                        console.log("Voice control is disabled in settings");
                        return;
                    }
                    ...
                    if (validCommands.includes(command.toLowerCase())) {
                        ...
                        // Check if command matches an animation name
                        var animationNames = Object.keys(_this.animationActions);
                        console.log("Available animations:", animationNames);
                        console.log("Trying to match command:", command);
                        
                        var matchedAnimation = animationNames.find(function(name) {
                            var nameLower = name.toLowerCase();
                            var commandLower = command.toLowerCase();
                            // Try exact match, contains, or starts with
                            return nameLower === commandLower || 
                                   nameLower.includes(commandLower) ||
                                   commandLower.includes(nameLower) ||
                                   nameLower.startsWith(commandLower) ||
                                   commandLower.startsWith(nameLower);
                        });
                        
                        if (matchedAnimation) {
                            console.log("Playing animation via voice: ".concat(matchedAnimation));
                            // Switch to animate mode first if not already in it
                            if (_this.interactionMode !== 'animate') {
                                _this._setInteractionMode('animate');
                            }
                            _this._playAnimation(matchedAnimation);
                        } else {
                            console.warn("Unrecognized command via speech: ".concat(command));
                            console.warn("Available animations are:", animationNames.join(', '));
                        }
                    }
                });
